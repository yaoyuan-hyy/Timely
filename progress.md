...
4. 优化自然语言删除的多轮消歧体验。
5. 为日历页补更接近真实交互的端到端/截图验证，覆盖已取消记录开关、恢复和彻底删除。
6. 继续压缩 `components/timely-app.tsx` 的业务职责，把 AI 请求和事件操作抽成更清晰的本地 hook 或 service。
7. 流水记录后续可继续补充编辑、删除、筛选和更细分类；真实语音输入、提醒和云同步仍放在后续评估。
